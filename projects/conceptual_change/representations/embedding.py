...
class Embedding:
    ...
    # Adding function to get nearest nieghbors within radius

    def neighbors_within_radius(self, w, radius = 0.5):


        scores = self.m.dot(self.represent(w))
        neighbors_within_radius = [(cos, word) for cos, word in zip(scores, self.iw) if cos > radius]

        return neighbors_within_radius



    # Adding a function to return nearest neighbor of a composed vector:
    def closest_composed(self, w_vectors, n=10):
        """
        Assumes the vectors have been normalized.
        """
...
class SVDEmbedding(Embedding):
    ...
    def __init__(self, path, normalize=True, eig=0.0, **kwargs):
        ut = np.load(path + '-w.npy')
        s = np.load(path + '-w.npy')
